...
> Hola, soy Bit, tu ajolote guia. Hasta ahora hablamos de SQL y de Postgres como si vivieran en una computadora misteriosa muy lejana. En este capitulo le ponemos cara a esa computadora: **Supabase**. Es la herramienta que **RachaSimple** y **Faro** usan para guardar habitos, rachas, proyectos y fases. Vamos a ver que es, como se habla con ella desde el navegador y por que hay dos llaves que NUNCA debes confundir. Respira hondo: esto es mas amigable de lo que parece.

## 1. ¿Que es Supabase?

Imagina que Postgres (la base de datos que ya conoces) es un almacen enorme con estanterias muy ordenadas. El problema es que un almacen, por si solo, no atiende clientes: necesitas puertas, mostradores, guardias de seguridad y un telefono para pedir cosas. **Supabase** es todo ese servicio alrededor del almacen, ya montado y listo para usar.

> ### 🟦 ¿Que significa? — *Supabase*
> Una plataforma que te da una base de datos **Postgres** ya instalada y administrada (no tienes que mantener el servidor tu mismo) y, ademas, una serie de **APIs** automaticas para leer y escribir esos datos desde tu aplicacion. Tambien incluye login de usuarios, almacenamiento de archivos y mas.
> **Para que sirve:** te ahorra montar y cuidar un servidor de base de datos; tu te concentras en tu app.
...
> "Gestionado" quiere decir que alguien mas (en este caso Supabase) se encarga de las copias de seguridad, las actualizaciones y de que el servidor siga encendido. Tu solo usas la base de datos.
> **Para que sirve:** tener Postgres de verdad sin ser experto en administrar servidores.
> **Donde se usa en un repo real:** tanto **RachaSimple** como **Faro** corren sobre Postgres gestionado por Supabase.

> ### 🟦 ¿Que significa? — *API*
> Siglas de *Application Programming Interface*. Es un conjunto de "puertas" con reglas claras por donde tu programa pide datos o envia datos a otro sistema. Como el mostrador de un restaurante: pides por el menu, no entras a la cocina.
> **Para que sirve:** que tu app (el navegador) pueda pedirle a Supabase "dame los habitos de este usuario" sin tocar Postgres directamente.
...
> La parte de la app con la que el usuario interactua directamente: lo que se ve y se toca en el navegador (botones, listas, pantallas). Se opone al *servidor* (la parte oculta que guarda secretos y hace tareas pesadas).
...
Para contrastar: **PolyPaw** NO usa nada de esto. PolyPaw guarda sus datos en **archivos JSON** dentro del propio programa (Python/Flet). No hay un almacen central ni un mostrador: cada quien lee y escribe su archivo. Funciona para una app de escritorio, pero no sirve cuando muchas personas necesitan ver y actualizar los mismos datos al mismo tiempo. Para eso brilla Supabase.

> ### 🟦 ¿Que significa? — *JSON*
> Un formato de texto para guardar datos organizados en parejas "nombre: valor" y listas, facil de leer tanto para personas como para programas. Un archivo JSON es solo texto: se abre, se lee y se escribe sin necesidad de un servidor.
...
> **Python** es un lenguaje de programacion muy legible, popular para apps de escritorio y para tareas de datos. **Flet** es una herramienta que permite construir interfaces graficas (ventanas, botones) usando Python.
> **Para que sirve:** crear una app de escritorio con su propia ventana sin tener que ser un navegador web.
...
> ### 💡 Tip
> Una buena regla mental: si tus datos viven en un **archivo** que se abre con el programa (como en PolyPaw), tienes "datos locales". Si viven en un **servidor** al que muchos clientes se conectan (como en RachaSimple y Faro), tienes "datos en base de datos". Supabase es lo segundo.
...
> **JavaScript** es el lenguaje de programacion que corre en los navegadores y da vida a las paginas web. **TypeScript** es JavaScript con "etiquetas de tipo" añadidas (texto, numero, etc.) que ayudan a detectar errores antes de ejecutar; al final se convierte en JavaScript normal.
...
> El objeto que creas una vez con tu URL y tu llave, y que luego usas para todas tus consultas. Es como el telefono ya configurado: marcas y del otro lado contesta tu base de datos.
> **Para que sirve:** centralizar la conexion para no repetir configuracion en cada pantalla.
...
> Valores de configuracion (como una URL o una llave) que NO se escriben dentro del codigo, sino que se guardan aparte, fuera del programa, y este los lee al arrancar. Asi el mismo codigo funciona en tu maquina y en el servidor sin tocar nada, y los secretos no terminan dentro del codigo que se sube a GitHub.
...
> Un marco de trabajo (framework) construido sobre **React** para crear aplicaciones web que corren parte en el navegador y parte en un servidor. Trae reglas listas para rutas, paginas y manejo de secretos.
...
> ### 🔎 En tu codigo
> En **Faro** (Next.js) las variables que empiezan con `NEXT_PUBLIC_` son las unicas que el navegador puede ver. La URL del proyecto y la llave `anon` van con ese prefijo justamente porque son seguras de exponer. Cualquier secreto de verdad (como la llave de OpenAI o la `service_role`) NO lleva ese prefijo y se queda en el servidor.

## 3. Leer datos: .from().select()

Toda consulta empieza eligiendo una tabla con `.from(...)` y luego diciendo que columnas quieres con `.select(...)`.
...
Fijate en `{ data, error }`: Supabase siempre devuelve un objeto con esos dos campos. Si todo salio bien, `data` trae tus filas y `error` es `null`. Si algo fallo, `error` trae el problema y `data` es `null`. Revisa SIEMPRE el error.

> ### 🟦 ¿Que significa? — *fila*
> Cada registro de una tabla: un habito, una racha, un proyecto. Si la tabla es una hoja de calculo, una fila es un renglon.
...
Si lo tradujeramos al SQL puro que esa consulta ejecuta por debajo, seria algo asi:
...
> ### 💡 Tip
> El cliente `supabase-js` no reemplaza al SQL: lo **traduce**. Cada cadena de metodos que escribes se convierte en una consulta SQL real contra Postgres. Por eso vale la pena entender SQL: te ayuda a saber que esta pasando "del otro lado del telefono".

## 4. Filtros: afinar lo que pides

Casi nunca quieres TODAS las filas. Quieres las de un usuario, las completadas hoy, las mas recientes. Para eso estan los filtros, que encadenas despues del `.select()`.

> ### 🟦 ¿Que significa? — *.eq()*
> Filtro de igualdad: "donde tal columna sea **igual a** tal valor". `eq` viene de *equal*.
> **Para que sirve:** quedarte solo con las filas que coinciden exactamente (por ejemplo, de un solo usuario).
> **Donde se usa en un repo real:** Faro usa `.eq('proyecto_id', id)` para traer solo las fases de un proyecto concreto.

> ### 🟦 ¿Que significa? — *.gt()*
> Filtro "mayor que" (*greater than*). Tambien existen `.gte` (mayor o igual), `.lt` (menor que) y `.lte` (menor o igual).
...
> **Para que sirve:** mostrar lo mas reciente o lo mas alto primero.
...
Un ejemplo encadenando varios filtros, sobre los registros de racha de RachaSimple:
...
> ### 🔎 En tu codigo
> Nota como el orden de los metodos en `supabase-js` (primero `.eq`, luego `.gt`, luego `.order`, luego `.limit`) se corresponde con las clausulas de SQL (`where`, `order by`, `limit`). No es casualidad: cada metodo agrega una pieza a la misma consulta.

> ### ⚠️ Cuidado
> Si te olvidas de un filtro como `.eq('usuario_id', ...)`, podrias pedir filas que no son tuyas. En RachaSimple y Faro eso no rompe la seguridad porque hay **RLS** (lo vemos en la seccion 8), pero igual es mala costumbre traer mas de lo necesario. Pide poco y especifico.
...
> Metodo que le dice a Supabase "espero exactamente una fila". En vez de devolverte una lista con un elemento, te devuelve directamente el objeto. Si hay cero filas o mas de una, te da un error.
> **Para que sirve:** trabajar comodo con `data.nombre` en lugar de `data[0].nombre`.
> **Donde se usa en un repo real:** RachaSimple lo usa al traer el **perfil** del usuario actual, que es siempre uno solo.
...
> ### 💡 Tip
> Existe tambien `.maybeSingle()`. Es como `.single()` pero **no** da error si no hay ninguna fila: simplemente te devuelve `null`. Util cuando no estas seguro de que el registro exista todavia (por ejemplo, un perfil que aun no se ha creado).

## 6. Escribir datos: insert, update y delete

Leer es la mitad. La otra mitad es crear, modificar y borrar.
...
> **Para que sirve:** guardar algo nuevo: un habito, un registro de racha de hoy, un proyecto.
...
> Metodo para **modificar** filas que ya existen. Casi siempre va acompañado de un filtro (como `.eq`) para decir CUALES filas cambiar.
> **Para que sirve:** editar datos: renombrar un habito, cambiar el estado de un proyecto.
> **Donde se usa en un repo real:** Faro usa `.update(...)` para cambiar el estado o el progreso de un proyecto tras un analisis.
...
> ### ⚠️ Cuidado
> Un `.update()` o un `.delete()` **sin filtro** afecta a TODAS las filas de la tabla. Imagina borrar todos los habitos de todos los usuarios de un solo golpe. Siempre, siempre acompaña update y delete con un `.eq(...)` (u otro filtro). Bit ha visto ajolotes llorar por olvidar el `where`.
...
RachaSimple no llama a Supabase "a pelo" en cada pantalla. Usa una libreria que organiza las llamadas.
...
> Una libreria de React que gestiona los datos que vienen de un servidor: los guarda en memoria (cache), sabe cuando estan "viejos" y los vuelve a pedir, y maneja los estados de "cargando" y "error" por ti.
> **Para que sirve:** evitar pedir lo mismo mil veces y mostrar spinners y errores sin escribir tanto codigo.
...
> ### 🟦 ¿Que significa? — *cache*
> Una copia temporal de datos que ya pediste, guardada cerca para no volver a pedirla. Como tener una foto de la pizarra en vez de ir a la pizarra cada vez.
...
> ### 🔎 En tu codigo
> En RachaSimple, un "query" de TanStack Query envuelve un `await supabase.from('habitos').select('*')`. La idea: TanStack Query se encarga del *cuando* y *como guardar*; Supabase se encarga del *de donde vienen* los datos. Trabajan en equipo.
...
> La llave "publica" de tu proyecto Supabase. Es la que puede vivir en el navegador. Por si sola NO da permisos especiales: cualquiera que la tenga solo puede hacer lo que tus reglas de seguridad permitan.
...
> **Para que sirve:** tareas de servidor que necesitan permisos totales (por ejemplo, un proceso administrativo).
...
> ### ⚠️ Cuidado
> NUNCA pongas la llave `service_role` en codigo del navegador ni la subas a GitHub. Si se filtra, cualquiera puede leer y borrar todos los datos de todos tus usuarios. La regla de seguridad de Faro lo dice claro: tokens y secretos solo en el servidor, jamas en el cliente ni commiteados.
...
> **Para que sirve:** que aunque todos compartan la misma llave `anon`, cada quien solo acceda a SUS datos.
...
> ### 🟦 ¿Que significa? — *token*
> Una "ficha" temporal que un servicio te entrega tras un login con OAuth. Sirve de permiso: la presentas en cada peticion para demostrar quien eres, y se puede vencer o revocar. No es tu contraseña.
> **Para que sirve:** permitir que la app actue en tu nombre por un rato, sin guardar tu contraseña.
...
Una politica de RLS, en SQL, se ve asi (ejemplo del estilo que protege los proyectos de Faro):
...
> **Para que sirve:** escribir reglas como "esta fila es tuya si tu id coincide con su dueño".
...
> ### 💡 Tip
> Piensa en la llave `anon` como la llave de la entrada de un edificio de apartamentos: te deja entrar al edificio, pero no a cualquier apartamento. RLS es la cerradura de cada apartamento: solo abre el tuyo. La `service_role` seria la llave maestra del conserje, que abre todo... por eso la guardas bajo siete llaves.

## 9. Realtime y Storage, a grandes rasgos

Supabase ofrece dos extras que conviene conocer aunque no entremos a fondo.

> ### 🟦 ¿Que significa? — *Realtime*
> Una funcion que avisa a tu app **en el momento** en que un dato cambia en la base, sin que tengas que preguntar una y otra vez. Te "suscribes" a una tabla y Supabase te empuja los cambios.
> **Para que sirve:** que dos personas vean lo mismo actualizado al instante, o que una pantalla refleje cambios sin recargar.
> **Donde se usa en un repo real:** RachaSimple podria usar Realtime para que una racha registrada en el telefono aparezca al toque en otra pantalla. (En la practica, gran parte del refresco lo resuelve TanStack Query.)
...
> ### 🔎 En tu codigo
> Faro tiene su propia filosofia de "refresco bajo demanda": el usuario dispara el analisis cuando quiere. Por eso no abusa de Realtime; prefiere pedir datos cuando hace falta. Conocer la herramienta no obliga a usarla siempre: cada proyecto elige.
...
5. Si el usuario edita el estado, el navegador hace `.update(...).eq('id', ...)`, RLS vuelve a verificar que sea su fila, y Postgres guarda el cambio.
...
Y PolyPaw, mientras tanto, sigue feliz leyendo su archivo JSON local, sin llaves, sin RLS, sin servidor. Cada herramienta para su mundo.

> ### 💡 Tip
> Si solo te llevas una idea de este capitulo, que sea esta: **anon + RLS** es la pareja que mantiene seguro el frontend; **service_role** se queda en el servidor y nunca sale. Confundir esas llaves es el error de seguridad numero uno con Supabase.
