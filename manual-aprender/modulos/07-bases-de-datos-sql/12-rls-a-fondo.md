...
> Hola, soy Bit, tu ajolote guia. En el capitulo anterior aprendiste a guardar datos en tablas de Postgres. Hoy toca una pregunta algo incomoda: si **todos** los usuarios de RachaSimple comparten la misma tabla de habitos... ¿que impide que Ana vea los habitos de Beto? La respuesta es **Row-Level Security**, y, sin exagerar, es la pieza de seguridad mas importante de cualquier app hecha con Supabase. Ven conmigo, que esto lo vamos a entender de verdad, ladrillo por ladrillo.

## 1. El problema: una sola tabla, muchos dueños

Piensa en la tabla `habitos` de **RachaSimple** (la app de habitos hecha con Supabase/Postgres). No existe una tabla por persona; hay **una sola tabla** donde caen los habitos de todo el mundo. Cada fila lleva una columna que indica de quien es:
...
Si no proteges nada, una consulta `select * from habitos` te devuelve **todo**: los tuyos y los de los demas. Y en una app web, donde el cliente (el navegador) habla casi directo con la base de datos a traves de la API de Supabase, eso seria un desastre de privacidad.

Aqui **PolyPaw** sirve de contraste perfecto. PolyPaw guarda sus datos en **archivos JSON** locales, no en una base de datos relacional compartida. Cada instalacion tiene su propio archivo: no hay "filas de otros usuarios" mezcladas, asi que el problema de "quien ve que fila" alli ni siquiera aparece. RLS es para el mundo de **tablas compartidas** (RachaSimple, Faro), no para el de **archivos por usuario** (PolyPaw).
...
Al ejecutar esto pasa algo que sorprende a los principiantes: la tabla queda **cerrada para todos**. Sin politicas, RLS bloquea **todo** acceso (salvo el del dueño de la tabla y la `service_role`, que veremos mas adelante). Dicho de otra forma: activar RLS no abre nada, lo **cierra**. Lo que viene despues es tu tarea: abrir, con cuidado, solo lo justo mediante **politicas**.

> ### 🟦 ¿Que significa? — *Habilitar RLS (enable row level security)*
> Es el interruptor que **activa** la revision fila por fila en una tabla. Mientras este apagado, las politicas que escribas **no se aplican** (la tabla esta abierta). Sirve como el "candado maestro". En RachaSimple, cada tabla sensible (`habitos`, `registros`, `perfiles`) tiene RLS habilitado; en Faro pasa igual con `proyectos`, `fases` y `user_connections`.

> ### ⚠️ Cuidado
> El fallo de seguridad mas comun en Supabase es **crear una tabla y olvidar encender RLS**. La tabla parece funcionar perfecto en tu app... y, mientras tanto, esta visible para cualquiera con la clave publica. Por eso Supabase te muestra una advertencia ("RLS disabled in public"). Cada vez que crees una tabla con datos de usuarios, tu reflejo debe ser: `enable row level security`.

> ### 🔎 En tu codigo
> En **Faro**, la regla de seguridad del proyecto (ver `CLAUDE.md`) dice que los tokens y secretos viven en el servidor o en `user_connections` **con RLS**. Esa coletilla "con RLS" no esta de adorno: es lo unico que impide que un usuario lea las conexiones OAuth de otro. Sin RLS, esa tabla expondria tokens ajenos.

## 3. Politicas: las reglas que abren la puerta

Con RLS encendido, la tabla queda cerrada. Para dejar pasar a la gente correcta usamos **politicas** (policies). Una politica es una regla escrita en SQL que responde a una pregunta: "para **esta** operacion (leer, insertar, actualizar, borrar), ¿que filas puede tocar este usuario?".

Cada politica se asocia a un **comando**: `SELECT`, `INSERT`, `UPDATE` o `DELETE`. Y se apoya en una de estas dos clausulas:
...
> **USING** mira filas que **ya existen** y decide si las dejas ver/cambiar/borrar. **WITH CHECK** mira el dato que **vas a escribir** y decide si lo dejas guardar. La diferencia es "lo que ya hay" frente a "lo que entra". Sirve, por ejemplo, para impedir que alguien inserte una fila a nombre de **otro** usuario (eso lo bloquea `WITH CHECK`).

## 4. La pieza magica: `auth.uid()`

Todas las politicas de RachaSimple y Faro giran alrededor de una misma funcion: `auth.uid()`. Devuelve el **identificador (UUID)** del usuario que esta haciendo la peticion en ese instante, sacado de su token de sesion. Es la forma que tiene la base de datos de preguntar: "¿quien eres tu, ahora mismo?".
...
> Un usuario **autenticado** es el que ya inicio sesion y tiene un token valido. Lo contrario es **anonimo** (`anon`), sin sesion. Sirve para distinguir visitantes de usuarios reales. En Supabase, las politicas suelen dirigirse al rol `authenticated`; un `anon` sencillamente no pasa, porque su `auth.uid()` es `null`.

## 5. El patron de oro: `usuario_id = auth.uid()`

Junta las piezas y aparece el patron que protege practicamente toda app de Supabase, RachaSimple y Faro incluidas:
...
Esto se lee asi: "la columna `usuario_id` de esta fila debe ser igual al id del usuario que esta consultando". Si coinciden, la fila es tuya y la ves; si no, queda invisible. Veamos las **cuatro** politicas reales de la tabla `habitos` de RachaSimple:
...
Mira con atencion el UPDATE: lleva **las dos** clausulas. `USING` garantiza que solo puedas tocar filas que ya son tuyas; `WITH CHECK` garantiza que, despues de editar, la fila **siga** siendo tuya (que no le pongas el `usuario_id` de otra persona). Sin ese `WITH CHECK`, alguien con malas intenciones podria "regalarle" su fila a otro o, peor todavia, robar la de alguien.
...
> ```
> De este modo, aunque el cliente no envie `usuario_id`, Postgres pone el del usuario actual y el `WITH CHECK` siempre cuadra.

> ### 🔎 En tu codigo
> En **Faro**, la tabla `fases` pertenece a un `proyecto`, y el proyecto pertenece a un usuario. Su politica no compara contra `auth.uid()` de forma directa, sino que comprueba que la fase cuelgue de un proyecto **tuyo**, usando una subconsulta. Ese patron de "la fila hija es mia si su padre es mio" lo veremos en un momento.

## 6. Cuando el dueño esta en otra tabla (Faro: proyectos y fases)

En Faro, una **fase** no guarda el `user_id` directamente; guarda el `proyecto_id`. El dueño esta en la tabla `proyectos`. Asi que, para saber si una fase es tuya, hay que mirar su proyecto. La politica recurre a una subconsulta `exists`:
...
Se lee asi: "deja ver esta fase **si existe** un proyecto con el id al que apunta la fase **y** ese proyecto es mio". De este modo, la pertenencia se hereda del padre. Es algo muy habitual cuando tienes tablas relacionadas (proyecto → fases, habito → registros).
...
> Es una columna que **apunta** a la fila de otra tabla (por ejemplo, `fases.proyecto_id` apunta a `proyectos.id`). Sirve para relacionar datos y asegurar que no apunten a la nada. En Faro, esa llave foranea es justo lo que la politica usa para rastrear quien es el dueño real de una fase.

> ### 💡 Tip
> Si una tabla hija (como `registros` en RachaSimple) guarda tambien su propio `usuario_id`, puedes saltarte la subconsulta y usar el patron simple `usuario_id = auth.uid()`. Es mas rapido (no hace falta consultar otra tabla) a cambio de duplicar el dato del dueño. Muchas apps lo eligen a proposito por rendimiento.

## 7. El peligro mayuscula: la `service_role`

Supabase te da varias claves. Ahora mismo nos importan dos:
...
> Es la clave **publica** que va en el navegador. Identifica a tu proyecto pero **no** reparte poderes: todo lo que pase con ella esta sujeto a RLS y al usuario autenticado. Sirve para que el front hable con Supabase de forma segura. RachaSimple y Faro la usan en el cliente sin riesgo, porque RLS la mantiene a raya.

> ### ⚠️ Cuidado
> **Nunca** pongas la `service_role` en codigo del cliente ni la commitees. En Next.js (Faro), una variable con prefijo `NEXT_PUBLIC_` se envia al navegador: jamas metas la `service_role` ahi. La regla de seguridad de Faro lo dice sin rodeos: *"Tokens y secretos solo en el servidor. Nunca exponer claves en el cliente ni commitearlas."* La `service_role` es el ejemplo numero uno de esa regla.
...
Una politica mal escrita puede dejar la puerta abierta sin que lo notes: la app "funciona" igual. Por eso conviene **probar** activamente. Hay tres formas, de menos a mas realista:

**a) Probar como un usuario concreto desde el editor SQL de Supabase.** Puedes simular el rol y el id de un usuario y ver que te devuelve una consulta:
...
**b) La prueba del intruso.** Pon el id de Ana en la sesion y pide a proposito un habito de Beto. El resultado correcto son **cero filas** (no un error: simplemente la fila es invisible):
...
**c) Probar desde la app real con dos cuentas.** Crea dos usuarios de prueba, inicia sesion con cada uno y confirma que ninguno ve lo del otro. Como RachaSimple usa **TanStack Query** para traer datos, asegurate de que los datos cacheados de una cuenta no se cuelen al cambiar de usuario.
...
> ### 💡 Tip
> Una forma rapida de cazar tablas sin proteger: en el panel de Supabase, abre **Advisors** (o la pestaña de seguridad). Te lista las tablas con RLS apagado. Si ahi aparecen `proyectos`, `fases`, `habitos` o `user_connections`, corre a encenderlo.

> ### ⚠️ Cuidado
> Que tu app muestre solo tus datos **no** demuestra que RLS funcione. A lo mejor tu codigo añade un `.eq('usuario_id', miId)` y por eso ves solo lo tuyo. Pero si RLS esta apagado, un atacante puede ignorar tu codigo y pedir todo por la API. **El filtro en el front es comodidad; RLS es la seguridad real.** Prueba siempre a nivel de base de datos.
...
- **Prueba** siempre con la "prueba del intruso", no te fies del filtro del front.
