...
> Hasta ahora tu NAS **polypaw-nas** ha funcionado casi por arte de magia: enciendes el viejo Acer Nitro, abres Cockpit en el navegador y ahi esta. Pero detras de esa magia hay una red, con sus reglas, sus direcciones y sus puertas. En este capitulo abrimos esa caja con calma. Vas a entender quien es quien en tu casa digital: que es una IP, por que tu NAS necesita una direccion fija, que hace el router, como funciona el DNS (y por que tu polypaw-nas tiene su propio servidor DNS con AdGuard Home), que son los puertos y por que abrir uno al internet sin pensarlo es como dejar la puerta de la calle abierta de par en par. Bit, el ajolote, te acompana. Es chiquito, vive en el agua y tiene branquias rosas: el animal perfecto para recordarte que en redes todo respira tranquilo si no te apuras.
...
> IP viene de *Internet Protocol*. Una direccion IP es un numero que identifica a un aparato en una red, como `192.168.1.42`. Son cuatro numeros del 0 al 255 separados por puntos.
...
> ### 🟦 ¿Que significa? — *SSH*
> SSH viene de *Secure Shell*: caparazon seguro. Es una forma de abrir la terminal de otro equipo de forma remota y cifrada, escribiendo comandos en polypaw-nas desde tu laptop sin sentarte frente al Acer.
> **Para que sirve:** administrar el servidor por consola sin tener teclado y pantalla conectados a el; todo lo que escribes viaja cifrado por la LAN.
...
Veras una linea parecida a `inet 192.168.1.42/24`. Ese numero antes de la barra es la IP. El `/24` lo explicamos en un momento, ten paciencia (Bit ya esta nadando hacia alla).
...
> Es la direccion que tu casa entera muestra **al mundo de internet**. Te la asigna tu proveedor de internet (la compania del cable o la fibra). Toda tu casa comparte una sola IP publica.
> **Para que sirve:** que internet sepa devolver respuestas a tu hogar. Cuando ves un video, la pagina responde a tu IP publica.
> **Donde aparece en tu NAS:** NO aparece directamente. polypaw-nas no tiene IP publica propia. Y eso, te adelanto, es una **buena noticia para tu seguridad**.

La analogia: tu **IP publica** es la direccion de tu edificio que ve el cartero del mundo. Tus **IP privadas** son los numeros de apartamento adentro. El cartero deja todo en la porteria (tu router) y la porteria reparte adentro.

> ### 💡 Tip
> Para ver tu IP publica, desde polypaw-nas ejecuta `curl ifconfig.me`. Veras un numero distinto al `192.168.x.x`. Ese es el rostro de tu casa hacia internet. No lo publiques por ahi sin razon.

> ### 🔎 En tu servidor
> Como polypaw-nas solo tiene IP privada, **nadie desde internet puede tocarlo directamente**. Para llegar a el de forma remota usas Tailscale, que te crea una red privada virtual segura sin abrir nada al mundo. Volveremos a esto en la seccion de seguridad: es el corazon de por que tu NAS esta tranquilo.

---

## 3. La mascara de red y el rango de tu LAN

Vuelve a aquel `192.168.1.42/24`. Ese `/24` es la **mascara de red**, y define hasta donde llega tu vecindario local.
...
> **Donde aparece en tu NAS:** el `/24` en la salida de `ip addr` de polypaw-nas. Tambien se escribe como `255.255.255.0`, que es la misma cosa con otra notacion.

Con `/24`, tu rango de direcciones va de `192.168.1.1` a `192.168.1.254`. Eso te da espacio para unos 254 aparatos en casa, mas que suficiente.

> ### 🟦 ¿Que significa? — *Rango LAN*
> Es el conjunto de todas las direcciones validas de tu red local. Con mascara `/24` y red `192.168.1.x`, el rango util es `.1` a `.254`.
> **Para que sirve:** saber que direcciones puedes repartir. Tu router suele tomar la `.1`, y reparte el resto a los aparatos.
...
> ### ⚠️ Cuidado
> Tu red podria no ser `192.168.1.x`. Muchos routers usan `192.168.0.x`, `192.168.100.x` o `10.0.0.x`. **No copies mis numeros a ciegas:** mira los reales de tu casa con `ip addr show` en polypaw-nas. Bit insiste: en redes, primero miras, despues escribes.

---

## 4. El router y la puerta de enlace (gateway)

En medio de toda tu red hay un aparato que es a la vez jefe, portero y traductor: el **router**.

> ### 🟦 ¿Que significa? — *Router*
> Es la caja que tu proveedor de internet te dio (o que compraste). Conecta tu LAN con internet y reparte la conexion entre todos tus aparatos, por cable y WiFi.
...
> **Donde aparece en tu NAS:** polypaw-nas se conecta a internet **a traves de tu router**. Cuando el NAS descarga una actualizacion de Ubuntu, pide los datos al router y este los trae de internet.

Cuando un aparato quiere hablar con algo que NO esta en tu LAN (por ejemplo, una pagina web), no sabe el camino. Asi que entrega el paquete a una direccion especial: la **puerta de enlace**.
...
> **Para que sirve:** ser la unica puerta hacia internet. Si un aparato no sabe a donde mandar algo, se lo da a la gateway y ella se encarga.
...
> ### 💡 Tip
> Si conoces la IP de tu router (la gateway), puedes abrir su panel de configuracion escribiendola en el navegador: `http://192.168.1.1`. Ahi se hacen cosas importantes que veremos luego, como reservar la IP del NAS. Te pedira usuario y contrasena; suelen estar en una pegatina debajo del router.

---

## 5. DNS, con mucha calma

Aqui viene el concepto que mas nervios da y menos razon tiene de darlos. Respira con Bit, que el DNS es facil cuando lo cuentas bien.

Los humanos recordamos nombres: `google.com`, `github.com`. Las maquinas, en cambio, solo entienden numeros (IP). Hace falta una agenda telefonica que traduzca nombres a numeros. Esa agenda es el **DNS**.
...
> Es un programa que actua como tu servidor DNS personal y que, ademas de traducir nombres, **bloquea** los nombres de servidores conocidos de anuncios y rastreadores.
...
> ### 🔎 En tu servidor
> Para que AdGuard Home proteja toda tu casa, hay que decirle al **router** que el servidor DNS de la red es la IP de polypaw-nas. Asi todos los aparatos preguntan al NAS, y el NAS filtra anuncios para todos. Si solo lo configuras en un aparato, solo ese queda protegido.

> ### ⚠️ Cuidado
> Si polypaw-nas se apaga y tu router apunta a el como unico DNS, **toda la casa se queda sin internet** (nadie puede traducir nombres). Por eso, en el router conviene poner un DNS de respaldo, como `1.1.1.1` (Cloudflare) o `9.9.9.9` (Quad9), por si tu NAS descansa. Recuerda que la bateria interna del Acer Nitro hace de UPS natural (una *Uninterruptible Power Supply*, o fuente de alimentacion ininterrumpida: una bateria que mantiene el equipo vivo durante un corte de luz), asi que un apagon corto no lo tumba, pero un mantenimiento si.
...
Tu polypaw-nas hace muchas cosas a la vez: comparte archivos por Samba, muestra el panel Cockpit, resuelve DNS con AdGuard. Todo eso en una sola IP. Como sabe el NAS a cual servicio va cada peticion? Con los **puertos**.

> ### 🟦 ¿Que significa? — *Puerto*
> Es un numero (del 1 al 65535) que identifica a un servicio concreto dentro de un aparato. Si la IP es la direccion del edificio, el puerto es el numero de la oficina adentro.
...
> Es el puerto estandar del servicio DNS. AdGuard Home escucha aqui las preguntas "que IP tiene este nombre?".
...
Veras una tabla. Busca en la columna de direcciones los numeros `:445`, `:9090` y `:53`. A su lado aparece el programa que escucha (smbd, cockpit, AdGuardHome). Es como pasar lista de las puertas abiertas adentro de tu NAS.

> ### 💡 Tip
> La columna "State" mostrara `LISTEN` para los servicios que estan esperando conexiones. Si un servicio que esperabas no aparece, quiza no esta arrancado. Con Cockpit o con `systemctl status` puedes revisar y reiniciarlo.

---

## 7. Reservar una IP fija para tu NAS (DHCP reservation)

Tenemos un problema practico que resolver. Hasta ahora tu router reparte IP automaticamente con un sistema llamado DHCP. Eso es comodo, pero tiene un riesgo: la IP de polypaw-nas **podria cambiar** al reiniciar.
...
> **Para que sirve:** que conectes aparatos sin configurar IP a mano. Genial para el telefono y la tele.
> **Donde aparece en tu NAS:** cuando polypaw-nas arranca, pide una IP al router por DHCP. El problema: el router podria darle hoy `.42` y manana `.57`.
...
> **Donde aparece en tu NAS:** lo configuras en el panel del router, no en el NAS. Una vez hecho, la IP de polypaw-nas no vuelve a cambiar.
...
> **Para que sirve:** que el router reconozca a polypaw-nas sin importar que IP tenga, para asignarle siempre la reservada.
...
## 8. Por que NO abrir puertos al internet a la ligera
...
> Es configurar el router para que las peticiones que lleguen de internet a un puerto se reenvien a un aparato concreto de tu LAN. En la practica, abres una puerta de tu casa hacia el mundo entero.
...
- En cuanto abres un puerto, **bots automaticos** lo encuentran en minutos. No hace falta que alguien te conozca; rastrean todo internet sin parar.
- Samba (445) expuesto al internet es una de las puertas mas atacadas de la historia. Por ahi entraron ataques famosos de secuestro de datos.
- Cockpit (9090) expuesto da acceso a **administrar tu servidor entero**. Si alguien adivina la contrasena, controla todo.
- DNS (53) abierto puede ser usado para amplificar ataques contra terceros, y meterte en problemas sin saberlo.

> ### ⚠️ Cuidado
> Nunca, jamas, abras los puertos 445, 9090 o 53 de polypaw-nas hacia internet "por probar". No es como dejar una ventana entornada: es como publicar la llave de tu casa en una valla de carretera. Si necesitas acceso remoto, hay una forma segura, y ya la tienes instalada.
...
> **Para que sirve:** acceder a polypaw-nas desde cualquier lugar **sin abrir ningun puerto** al internet. Solo tus propios aparatos, autenticados, pueden entrar al tunel.
