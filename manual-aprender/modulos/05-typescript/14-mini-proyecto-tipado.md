...
> ¡Hola otra vez! Soy **Bit**, tu ajolote programador. 🐾 ¿Te acuerdas de la lista de tareas que armaste en el Módulo 03 con JavaScript puro? Aquella libretita digital donde escribías "Comprar pan", la marcabas como hecha y la borrabas. Hoy vamos a hacer algo bonito: **la vamos a reescribir con TypeScript**. No para empezar de cero, sino para ponerle "etiquetas de seguridad" a cada pieza. Vas a ver con tus propios ojos el momento exacto en que TypeScript te salva de un error *antes* de ejecutar el programa. Y al final pondremos las dos versiones lado a lado —la de JavaScript sin tipos y la nueva con tipos— para que sientas por ti mismo qué aportan los tipos. Recuerda el lema de todo el módulo: **TypeScript es JavaScript con tipos**. Todo lo que ya sabes sigue valiendo; solo le sumamos avisos. Respira. Esto lo vas a disfrutar.

---

## 1. ¿Qué vamos a construir (otra vez)?

La misma lista de tareas de siempre: una caja para escribir, un botón para añadir, una lista donde aparecen las tareas, clic para marcarlas como hechas y un botón para borrarlas. **El comportamiento no cambia.** Lo que cambia es que ahora cada dato lleva un **tipo** que le dice a TypeScript (y a tu editor) qué forma tiene.

Quizá tengas una duda muy razonable: *"Si la versión de JavaScript ya funcionaba, ¿para qué complicarme?"*. La respuesta corta: para que el editor te avise de los errores mientras escribes, y no cuando se rompa en manos de un usuario. La respuesta larga es todo este capítulo.
...
> ### 💡 Tip
> No tienes que tirar la versión de JavaScript del Módulo 03. Lo ideal es tener las dos carpetas abiertas y compararlas. Verás que el 90% del código es **idéntico**. TypeScript no es un lenguaje nuevo que aprender de cero: es tu JavaScript de siempre con unas cuantas anotaciones encima.

## 2. Preparamos el proyecto

En el Módulo 03 abrías el `index.html` con doble clic y listo. Con TypeScript hay un paso de más: el navegador **no entiende** TypeScript directamente, así que primero hay que **traducirlo** a JavaScript. A ese paso se le llama **compilar**.
...
Para este mini-proyecto vamos a usar la forma más sencilla que hay. Crea una carpeta `lista-tareas-ts` y dentro un archivo `app.ts`. Si quieres compilar a mano, instala TypeScript una sola vez con `npm install -g typescript` y luego, dentro de la carpeta, ejecuta:
...
> ### 💡 Tip
> Si esto te suena a mucho lío, tranquilo: en proyectos reales nunca compilas a mano; las herramientas (Vite, Next.js) lo hacen solas mientras programas. Aquí lo hacemos manual una vez para echar un vistazo a la "cocina" por dentro. Lo que importa en este capítulo son los **tipos**, no la compilación.

El `index.html` y el `estilos.css` son **exactamente los mismos** del Módulo 03. Lo único que cambia es que el `<script>` apunta a `app.js` (el archivo ya traducido, no a `app.ts`).

## 3. El corazón del capítulo: el `type Tarea`

Aquí está la gran diferencia. En JavaScript, una tarea era simplemente esto:
...
Funcionaba, pero JavaScript no sabía absolutamente nada sobre esa tarea. Si por error escribías `tarea.echa` (sin la "h") o le ponías `tarea.hecha = "sí"` (un texto donde debía ir verdadero/falso), nadie te avisaba. El programa fallaba *después*, cuando ya era tarde.
...
> ### 🔎 En tu codigo
> Fíjate que en RachaSimple y Faro hay un archivo **dedicado solo a tipos** (`types/database.ts` y `lib/types.ts`). No es casualidad: definir las formas de tus datos en un único sitio y reutilizarlas es una de las costumbres más sanas de TypeScript. Tu `type Tarea` es la versión de juguete de esos archivos.
...
En JavaScript guardabas todas las tareas así:
...
El problema: ese `[]` está vacío y JavaScript no tiene ni idea de qué meterás dentro. Podrías meter tareas, números, texto suelto, lo que se te ocurra, y nadie protestaría. En TypeScript le decimos **qué contiene** el array:

```typescript
let tareas: Tarea[] = [];
```

Ese `: Tarea[]` significa "un array de `Tarea`". Ahora TypeScript sabe que dentro solo van tareas con su `texto` y su `hecha`. Si intentas hacer `tareas.push(42)`, el editor se pone rojo en el acto.
...
> ### 🔎 En tu codigo
> Esa idea de "fuente de la verdad" que aprendiste en el Módulo 03 sigue igual de viva: el array `tareas` manda, la pantalla se redibuja a partir de él. TypeScript no cambia esa filosofía. Solo le pone un guardia en la puerta del array que dice: "aquí solo entran objetos con forma de `Tarea`".

## 5. Tipar las funciones: añadir una tarea

Vamos con la función de añadir. Compara las dos versiones con calma.
...
Casi calcadas, ¿verdad? Solo cambian dos detalles diminutos, pero con mucha chicha:
...
Aquí llega lo más jugoso del capítulo. En el Módulo 03 buscabas los elementos del HTML así:
...
El problema escondido: `document.getElementById` **puede no encontrar el elemento** (si te equivocas en el `id`, o si el HTML aún no cargó). En ese caso devuelve `null`, que significa "aquí no hay nada". En JavaScript esto reventaba en tiempo de ejecución con el temido error *"Cannot read properties of null"*. En TypeScript, el editor te **obliga** a contemplar esa posibilidad.
...
> `string | null` significa "esto es un texto **o** está vacío (null)". Esa barrita `|` se lee "o". Es la forma que tiene TypeScript de decir "ojo, este valor podría no existir, tenlo en cuenta". Sirve para que no se te olvide manejar el caso "no hay nada". En **Faro**, `next_action: string | null` es justo esto: o hay una próxima acción, o no la hay.
...
TypeScript sabe que `entrada` es del tipo `HTMLElement | null`. Y en cuanto intentes hacer `entrada.value`, te lo subrayará: *"`entrada` es posiblemente null"*. ¡Te está protegiendo! La forma segura de manejarlo es comprobar primero:
...
Después de ese `if`, TypeScript hace algo casi mágico que se llama **estrechamiento**: como ya descartamos el `null`, de esa línea en adelante trata a `entrada` y `lista` como elementos que **sí existen**. Y deja de molestarte con avisos.
...
> Estrechar es cuando TypeScript, gracias a un `if`, **descarta posibilidades** y afina el tipo. Antes del `if`, `entrada` era "elemento o null"; después, TypeScript sabe que es "elemento" a secas. Sirve para que, una vez comprobado, puedas usar el valor sin más avisos. Lo viste a fondo en el Capítulo 06.
...
> ### ⚠️ Cuidado
> El `as` es una promesa tuya: le dices a TypeScript "confía en mí". Si te equivocas y eso **no** era un `<input>`, TypeScript ya no te cubre y el error vuelve en tiempo de ejecución. Úsalo solo cuando de verdad sepas la forma del dato. No es una varita para callar avisos molestos: es una afirmación seria.
...
> ### 🔎 En tu codigo
> ¿Notaste que dentro del `forEach`, al escribir `tarea.`, tu editor te ofrece solito `texto` y `hecha`? Eso se llama **autocompletado**, y es uno de los regalos de tipar tus datos. En RachaSimple, cuando alguien escribe `habit.`, el editor sugiere las 20+ propiedades de `Habit` sin que tengas que recordarlas de memoria. Tipar es también escribir más rápido y con menos errores de dedo.

## 7. Guardar y cargar: el viaje de ida y vuelta a texto

`localStorage` solo guarda **texto**, igual que en el Módulo 03. Usamos los mismos traductores de siempre, `JSON.stringify` y `JSON.parse`. La diferencia en TypeScript asoma al **cargar**:
...
Fíjate en dos cosas. Primero, `localStorage.getItem` devuelve `string | null` (puede que todavía no haya nada guardado), por eso el `if (guardado)` antes de usarlo: es manejo seguro de null otra vez. Segundo, `JSON.parse` devuelve un tipo "cualquiera" porque no puede adivinar la forma del texto guardado, así que con `as Tarea[]` le decimos "lo que sale de aquí son tareas".

> ### 🟦 ¿Qué significa? — *JSON.parse y los datos de fuera*
> `JSON.parse` convierte texto en datos, pero TypeScript **no puede saber** qué forma tienen esos datos hasta que se lo dices. Por eso usamos `as Tarea[]`. Sirve para tipar datos que vienen de "fuera" (archivos, almacenamiento, internet). En **Faro**, los datos que llegan de GitHub se guardan en tipos como `GithubSnapshot` justo por esto: lo de afuera hay que tiparlo a mano.

> ### ⚠️ Cuidado
> El `as Tarea[]` confía en que lo guardado sean tareas de verdad. Si alguien manipula el `localStorage` a mano y mete basura, TypeScript no lo detecta (recuerda: los tipos solo existen mientras programas, desaparecen al compilar). En apps serias como RachaSimple se **valida** lo que llega de fuera antes de fiarse de ello. Para tu mini-proyecto, el `as` está bien; solo que sepas que no es magia infalible.
...
**En la versión JavaScript del Módulo 03:** no pasa *nada* al escribirlo. El programa arranca tan feliz. Luego, cuando `pintarLista` intenta leer `tarea.hecha`, se encuentra con `undefined`, y la tarea nunca se tacha bien. Te pasas media hora buscando el error sin saber por dónde anda. 😵
...
**Ese** es el valor de los tipos. No hacen tu app más rápida ni más bonita. Hacen que **tú** cometas menos errores y los pilles antes. Es como tener a Bit mirando por encima de tu hombro todo el rato, soltando un "oye, eso no cuadra". 🐾

> ### 🔎 En tu codigo
> En proyectos pequeños el ahorro parece poca cosa. Pero RachaSimple tiene decenas de archivos y Faro maneja datos de GitHub, Google Drive, OpenAI y Supabase a la vez. Sin tipos, un cambio en la forma de un dato rompería cosas en diez sitios sin avisar. Con tipos, el editor te marca los diez sitios al instante. Cuanto más grande el proyecto, más te salvan los tipos. Por eso ambas apps están escritas enteras en TypeScript.
...
Y lo más importante: te diste cuenta de que **no aprendiste un lenguaje nuevo**. Reutilizaste todo tu JavaScript del Módulo 03 y le sumaste etiquetas de seguridad. Eso es exactamente lo que hacen, a gran escala, RachaSimple y Faro: el mismo JavaScript de siempre, pero con un guardia de tipos que vela por que todo encaje.
