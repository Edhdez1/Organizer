...
> Hasta aquí has aprendido HTML solito: etiquetas, texto, formularios, tablas, accesibilidad. Pero una página real nunca vive sola. El HTML es solo uno de tres amigos que trabajan juntos. En este capítulo verás cómo el HTML llama al CSS para vestir la página y al JavaScript para darle vida, en qué orden cargan, y qué es ese "DOM" del que tanto se habla. Bit, el ajolote pixelado, dice que este es el capítulo donde por fin se ve "la foto completa". Respira, que vamos despacio.

## 1. Tres amigos, tres trabajos

Imagina que construyes una casa. Necesitas tres cosas distintas: los **muros y las habitaciones** (la estructura), la **pintura y los muebles** (el aspecto), y los **interruptores y la puerta automática** (lo que reacciona cuando alguien toca algo). En una página web esos tres papeles tienen nombre:
...
> ### 🟦 ¿Qué significa? — *CSS*
> CSS son las siglas de *Cascading Style Sheets* ("hojas de estilo en cascada"). Es un lenguaje aparte del HTML que sirve para decir **cómo se ve** cada cosa: este texto en color naranja, este recuadro con bordes redondeados, esta sección con fondo oscuro.
...
> **¿Dónde se usa en tu proyecto?** En **tunal-digital** está en el archivo `sitio-web/styles.css`. Ahí empieza definiendo los colores de marca (el "carbón" oscuro del fondo, el naranja del amanecer).

> ### 🟦 ¿Qué significa? — *JavaScript (JS)*
> JavaScript es un lenguaje de programación que el navegador entiende y ejecuta. Mientras el HTML y el CSS solo "describen", JavaScript **actúa**: puede cambiar textos, abrir y cerrar menús, reaccionar a clics.
...
> Una página bien hecha mantiene los tres separados: el **qué** en el HTML, el **cómo se ve** en el CSS, el **qué hace** en el JS. Si mezclas todo en un solo archivo, funciona, pero después es un dolor de cabeza cambiar algo. Por eso tunal-digital tiene tres archivos distintos: `index.html`, `styles.css` y `main.js`.

Lo bonito es que **el HTML es el jefe de la reunión**: es el archivo que el navegador abre primero, y es el HTML quien dice "oye, también carga este CSS y este JavaScript". Veamos cómo lo hace.

## 2. Cómo el HTML llama al CSS

El HTML enlaza una hoja de estilos con la etiqueta `<link>`, que ya viste de pasada en el capítulo del `<head>`. Aquí está, sacada del archivo real de tunal-digital:

```html
<link rel="stylesheet" href="styles.css?v=20260605">
```

Vamos palabra por palabra, porque cada parte importa.

> ### 🟦 ¿Qué significa? — *etiqueta `<link>`*
> `<link>` es una etiqueta de HTML que conecta el documento con un archivo externo. No muestra nada en pantalla por sí misma; solo dice "tráete este recurso".
> **¿Para qué sirve?** El uso más común es enlazar la hoja de estilos CSS. También sirve para íconos, fuentes, etc.
...
> Un **atributo** es información extra que va dentro de una etiqueta. `rel` significa *relation* (relación) y explica **qué tipo de conexión** es. Cuando vale `rel="stylesheet"`, le dice al navegador: "este link es una hoja de estilos, aplícala".
> **¿Para qué sirve?** Para que el navegador sepa qué hacer con el archivo enlazado. Sin `rel="stylesheet"`, el navegador no entendería que ese CSS hay que aplicarlo.

> ### 🟦 ¿Qué significa? — *atributo `href`*
> `href` significa *hypertext reference* (referencia de hipertexto). Es **la dirección del archivo** que quieres traer: aquí, `styles.css`, que vive junto al `index.html`.
> **¿Para qué sirve?** Para indicar dónde está el recurso. Ya lo conocías de los enlaces `<a href="...">`; aquí cumple el mismo papel: apunta a un destino.

¿Y qué es ese `?v=20260605` al final? Es un truco muy útil.

> ### 🟦 ¿Qué significa? — *cache-buster (rompe-caché)*
> El navegador **guarda copias** de los archivos para no descargarlos cada vez (eso es la *caché*, su memoria temporal). El problema: si cambias el CSS, el usuario puede seguir viendo la copia vieja. Añadir `?v=20260605` (una "versión") hace que el navegador crea que es un archivo nuevo y lo vuelva a descargar.
> **¿Para qué sirve?** Para forzar que los visitantes vean tus cambios de estilo recientes.
> **¿Dónde se usa en tu proyecto?** En tunal-digital se usa en `styles.css?v=20260605` y también en los scripts: `main.js?v=20260607`. Cada vez que Edwar cambia el archivo, sube ese número.

> ### ⚠️ Cuidado — `<link>` no se cierra
> `<link>` es una etiqueta **vacía**: no lleva contenido dentro ni etiqueta de cierre `</link>`. Se escribe sola. Lo mismo pasaba con `<img>` y `<meta>`. Si intentas cerrarla, no rompe nada, pero no es necesario.

### ¿Dónde va el `<link>` del CSS?

Va **dentro del `<head>`**, antes de que empiece el `<body>`. ¿Por qué tan arriba? Porque quieres que los estilos estén listos **antes** de que el navegador dibuje el contenido. Si el CSS llegara tarde, verías por un instante la página "desnuda" (texto negro sobre blanco) antes de que se vista. Eso se llama, en jerga, "flash de contenido sin estilo", y queda feo.
...
El JavaScript se enlaza con la etiqueta `<script>`. En tunal-digital la línea real es:
...
> `<script>` es la etiqueta que carga y ejecuta código JavaScript en la página. Puede tener el código escrito dentro, o (lo más común y ordenado) apuntar a un archivo externo con el atributo `src`.
...
> `src` significa *source* (fuente, origen). Es **la dirección del archivo de código** que quieres cargar, igual que `href` lo es para el CSS. Aquí apunta a `main.js`.
...
> A diferencia de `<link>`, la etiqueta `<script>` **sí lleva cierre**: `</script>`. Aunque uses `src` y no escribas nada dentro, debes ponerlo: `<script src="main.js"></script>`. Si lo olvidas, el navegador se confunde y a veces "se come" el resto de la página. Bit ha visto a más de un principiante pelear una hora con esto.
...
Eso causa dos problemas:

1. **Lentitud:** mientras el script descarga, el usuario ve una página congelada.
2. **Elementos que aún no existen:** si tu JavaScript intenta tocar un botón que está más abajo en el HTML y el script se ejecutó antes de que ese botón existiera... no lo encuentra, y falla.
...
> **¿Dónde se usa en tu proyecto?** En tunal-digital, `main.js` se carga con `defer`. Por eso su código puede buscar el botón del menú o los enlaces de idioma con total confianza: para cuando corre, ya están todos ahí.

Existe un primo de `defer` llamado `async`. Conviene saber qué hace, aunque casi siempre quieras `defer`.
...
Aquí va la diferencia en una tabla mental sencilla:
...
> Para tu propio código (como `main.js`), usa **`defer`**. Es el que respeta el orden y garantiza que la página esté lista. Reserva `async` para scripts ajenos que no dependan de tu HTML. Y evita el `<script>` normal sin nada: es el más lento.
...
> ```
>
> Fíjate que `manifest.js` (los datos de la marca) va **antes** que `main.js` (la lógica que los usa). Con `defer`, el orden de escritura se respeta: primero se ejecuta uno, luego el otro. Si los hubieran cargado con `async`, ese orden no estaría garantizado y podría romperse.

### ¿Al final del `<body>` o con `defer`?

Históricamente, mucha gente ponía los `<script>` justo antes de `</body>` (al fondo de la página) para que el HTML cargara primero. Sigue siendo válido. Pero con `defer` ya no hace falta ese truco: puedes ponerlo donde quieras y se comportará bien. tunal-digital combina ambas costumbres: pone los scripts al final **y** les agrega `defer`, por seguridad y claridad. No es contradictorio, es ser cuidadoso.
...
> **¿Para qué sirve?** Es **el puente**. El CSS mira el DOM para saber a qué pintar de qué color. El JavaScript mira y **modifica** el DOM para cambiar la página en vivo: cambiar un texto, abrir un menú, ocultar una sección.
...
Piénsalo así: el HTML es la **receta escrita**; el DOM es el **plato ya servido en la mesa**, que aún puedes condimentar. El JavaScript es el cocinero que se acerca con la sal.

### Cómo JavaScript "encuentra" un elemento

Para que JavaScript pueda cambiar algo, primero tiene que **encontrarlo** en el DOM. Y para encontrarlo, el elemento HTML suele tener un `id` o una `class` que sirve de etiqueta de identificación.
...
> **¿Para qué sirve?** Para aplicar el mismo estilo o el mismo comportamiento a varios elementos de golpe. Por ejemplo, "todas las tarjetas con `class="glass"` van con fondo translúcido".
...
Mira cómo se conectan las tres piezas alrededor de un mismo elemento. Primero el HTML define un botón con un `id`:
...
Tres archivos, tres lenguajes, **un solo botón**. El `id="menuBtn"` es el hilo que los cose. Esa es, en una imagen, toda la integración.

> ### 💡 Tip — `id` para JavaScript, `class` para estilos repetidos
> Como `id` es único, va perfecto cuando hay **un solo** elemento (un botón concreto, una sección concreta). Cuando quieres aplicar el mismo estilo a **muchos** elementos (todas las tarjetas, todos los botones de un tipo), se usa `class`, que sí se puede repetir. tunal-digital usa muchísimas clases para sus tarjetas de "vidrio" (`class="glass"`).

## 5. La foto completa, paso a paso

Juntemos todo en el orden real en que ocurre cuando alguien abre tunal-digital en su navegador. Bit te lo narra como una pequeña película:
...
Aquí está el esqueleto mínimo que une los tres mundos, que podrías escribir tú mismo:
...
> "Vanilla" (vainilla, el sabor más básico) describe usar HTML, CSS y JavaScript **puros**, sin librerías ni herramientas que automaticen el enlazado. Es exactamente lo que hace tunal-digital.
...
En **RachaSimple** y **Faro** (la carpeta Organizer) no escribes el `<link>` y el `<script>` a mano: usas React, que reparte la página en piezas llamadas componentes (archivos `.tsx`), y una herramienta (Vite en RachaSimple, Next.js en Faro) se encarga de inyectar el CSS y el JS por ti en un HTML que genera automáticamente. Y para los estilos, RachaSimple usa Tailwind, que es CSS organizado de otra manera. Pero el resultado final que llega al navegador es siempre lo mismo: un HTML que enlaza CSS y JS, y un DOM que JavaScript manipula. **Lo que aprendiste aquí no caduca: es el cimiento de todo lo demás.**

> ### 💡 Tip — No corras todavía
> No necesitas entender React, Vite ni Next.js ahora mismo. Solo quédate con la idea: por mucho que cambie la herramienta, los tres amigos (HTML, CSS, JS) y el puente (DOM) siempre están. Cuando llegues a esos módulos, reconocerás el patrón.
