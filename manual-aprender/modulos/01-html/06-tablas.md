...
> En este capítulo vas a mostrar datos ordenados en filas y columnas con tablas de HTML. Verás cuándo una tabla es la herramienta correcta (datos como un horario o una lista de precios) y cuándo no toca usarla (para colocar cosas en la pantalla, que es trabajo de CSS). De paso aprenderás a hacer tus tablas accesibles para quienes navegan con lectores de pantalla. Bit, nuestro ajolote pixel art, viene contigo: hoy trae una libretita cuadriculada, que es justo la idea de una tabla.

## 1. ¿Qué es una tabla y para qué sirve?

Piensa en una hoja de cálculo o en el horario de clases pegado en la pared: tiene **filas** (líneas horizontales) y **columnas** (líneas verticales), y en cada cruce hay un dato. Eso, ni más ni menos, es una **tabla**.

> ### 🟦 ¿Qué significa? — *Tabla*
> Una **tabla** organiza información en filas y columnas, y cada celda guarda un dato relacionado con su fila y su columna. **¿Para qué sirve?** Para mostrar **datos tabulares**: información que cobra sentido al compararse por columnas, como precios, horarios, resultados o estadísticas.

> ### 🟦 ¿Qué significa? — *Datos tabulares*
> Son datos que **naturalmente** caben en una cuadrícula: cada fila es un elemento (por ejemplo, un servicio) y cada columna es una característica de ese elemento (su precio, su duración). **¿Para qué sirve la palabra?** Para que reconozcas cuándo sí va una tabla: si tus datos se parecen a una hoja de Excel, es tabla.

La regla de oro cabe en una frase: **usa una tabla cuando los datos quedarían igual de claros en una hoja de cálculo.** Si pudieras pasarlos a Excel sin perder sentido, una tabla de HTML les viene como anillo al dedo.

> ### 💡 Tip — La prueba del Excel
> Antes de armar una tabla, pregúntate: "¿Esto cabría bien en una hoja de cálculo?". Si la respuesta es sí (un menú con precios, un horario, una comparación de planes), adelante. Si la respuesta es "no, solo quiero acomodar una imagen al lado de un texto", entonces no es una tabla: es maquetación, y eso se hace con CSS (lo verás en el módulo de CSS).

## 2. La estructura básica: `table`, `tr`, `td`

Toda tabla de HTML arranca con la etiqueta `<table>`. Dentro van las filas, y dentro de cada fila, las celdas. Veamos las tres etiquetas mínimas.

> ### 🟦 ¿Qué significa? — *Etiqueta `<table>`*
> Es la etiqueta que **abre y cierra** la tabla completa. Todo lo demás (filas y celdas) vive dentro de ella. **¿Para qué sirve?** Le avisa al navegador: "aquí empieza una cuadrícula de datos".

> ### 🟦 ¿Qué significa? — *Etiqueta `<tr>` (table row)*
> `tr` viene del inglés *table row*, "fila de tabla". Cada `<tr>` es **una fila horizontal** completa. **¿Para qué sirve?** Para agrupar las celdas que comparten la misma línea.

> ### 🟦 ¿Qué significa? — *Etiqueta `<td>` (table data)*
> `td` viene de *table data*, "dato de tabla". Cada `<td>` es **una celda** con un dato adentro. **¿Para qué sirve?** Es el cajoncito donde escribes el contenido: un texto, un número, hasta una imagen.

Empecemos con el ejemplo más pequeño que existe: una tabla de dos filas y dos columnas.
...
Lee el código de arriba hacia abajo, igual que lo hace el navegador:

- La primera `<tr>` es la primera fila. Dentro lleva dos `<td>`: "Servicio" y "Precio".
...
En el ejemplo anterior, la primera fila eran títulos ("Servicio", "Precio"), no datos normales. Para esos títulos de columna o de fila, HTML tiene una etiqueta especial: `<th>`.

> ### 🟦 ¿Qué significa? — *Etiqueta `<th>` (table header)*
> `th` viene de *table header*, "encabezado de tabla". Se usa en vez de `<td>` cuando la celda es un **título** (el nombre de una columna o de una fila). **¿Para qué sirve?** El navegador la muestra en **negrita y centrada** por defecto y, lo más importante, los lectores de pantalla la anuncian como encabezado, lo que ayuda a las personas ciegas a entender la tabla.

Además, una tabla bien hecha se reparte en dos zonas: la **cabecera** (los títulos) y el **cuerpo** (los datos). Para marcar esa división están `<thead>` y `<tbody>`.

> ### 🟦 ¿Qué significa? — *Etiqueta `<thead>` (table head)*
> Agrupa la fila (o las filas) de **encabezados** de la tabla. **¿Para qué sirve?** Para separar, visual y semánticamente, los títulos del resto. "Semántico" quiere decir que aporta significado: el navegador y los buscadores entienden "esto son los encabezados".

> ### 🟦 ¿Qué significa? — *Etiqueta `<tbody>` (table body)*
> Agrupa las filas con los **datos reales**, el cuerpo de la tabla. **¿Para qué sirve?** Para dejar claro dónde está la información, aparte de los títulos. Y si más adelante usas CSS o JavaScript, poder apuntar al `<tbody>` te facilitará la vida.
...
¿Notas la diferencia? Los títulos van con `<th>` dentro de `<thead>`, y los datos con `<td>` dentro de `<tbody>`. Esta es la forma profesional, y la que conviene que uses siempre.
...
Una tabla suelta, sin explicar de qué trata, deja a cualquiera con dudas. Para darle un título visible que forme parte de la propia tabla, usamos `<caption>`.

> ### 🟦 ¿Qué significa? — *Etiqueta `<caption>`*
> Es el **título de la tabla**. Va justo después de abrir `<table>`, como primera línea de su contenido. **¿Para qué sirve?** Para que cualquiera (lectores de pantalla incluidos) sepa de un vistazo qué muestra la tabla. Es como ponerle nombre a una foto.
...
> El `<caption>` debe ser el **primer hijo** de `<table>`, antes de `<thead>`. Si lo pones en otro sitio, el navegador puede ignorarlo o moverlo. Regla fácil: abres `<table>` y lo siguiente es el `<caption>`.

## 5. Celdas que se estiran: `colspan` y `rowspan`

A veces una celda necesita ocupar el espacio de varias. Por ejemplo, un título que abarca dos columnas, o una celda que cubre dos filas. Para eso hay dos **atributos**.

> ### 🟦 ¿Qué significa? — *Atributo*
> Un **atributo** es información extra que se escribe **dentro** de la etiqueta de apertura, con la forma `nombre="valor"`. **¿Para qué sirve?** Para modificar cómo se comporta o se ve un elemento. Ya viste atributos como `href` en los enlaces; aquí toca `colspan` y `rowspan`.
...
La celda "Web" tiene `rowspan="2"`, así que se estira hacia abajo y cubre las dos filas. Por eso la segunda `<tr>` solo lleva **dos** celdas: la primera columna ya la ocupa "Web".
...
Esta sección importa porque, hace muchos años, los programadores usaban tablas **para colocar cosas en la página**: un menú a la izquierda, el contenido en el centro, etc. Hoy eso se considera un **error**.
...
¿Por qué está mal maquetar con tablas? Tres razones, contadas fácil:
...
3. **No se adapta bien al móvil.** Las tablas son rígidas. CSS, en cambio, trae herramientas (como Flexbox y Grid) pensadas para que la página se vea bien en cualquier pantalla.
...
> La **accesibilidad** es hacer que tu sitio lo pueda usar **todo el mundo**, incluidas las personas con discapacidad (por ejemplo, quienes navegan con un **lector de pantalla**, un programa que lee la página en voz alta). **¿Para qué sirve?** Para no dejar a nadie afuera. Es responsabilidad de quien programa, no un extra opcional.

En una tabla, los `<th>` son los encabezados, pero a veces el navegador no sabe si un encabezado manda sobre una **columna** o sobre una **fila**. Para dejarlo clarísimo usamos el atributo `scope`.
...
Aquí tienes una tabla accesible con `scope`:
...
> Añadir `scope`, `caption`, `<thead>` y `<tbody>` casi no cuesta mientras construyes la tabla, pero es un fastidio agregarlo después. Hazlo bien desde el primer momento y tu yo del futuro (y tus usuarios) te lo agradecerán. Bit asiente con sus branquias rosas: ajolote feliz, código accesible.

## 8. ¿Y los bordes? Una nota sobre el aspecto visual

Si pruebas las tablas de arriba en el navegador, notarás algo curioso: **no tienen líneas**. Las celdas están ahí, pero no se ven los bordes que dibujarías a mano en una cuadrícula. Esto desconcierta a mucha gente que empieza. Tranquilo: tu tabla está bien; lo que pasa es que HTML, por sí solo, casi no pinta líneas.
...
Quizá en tutoriales viejos viste algo como `<table border="1">`. Funcionaba, pero hoy suena anticuado: mezclar diseño (las líneas) con contenido (los datos) es justo lo que queremos evitar. La regla que ya conoces vuelve a aplicar.
...
Antes del repaso final, miremos las trampas más comunes al hacer tablas. Reconocerlas ahora te ahorrará dolores de cabeza.
...
> Usa `<th>` **solo** para títulos (nombres de columna o de fila) y `<td>` para los datos. Si pones todo con `<th>`, el lector de pantalla creerá que todo es encabezado y la tabla pierde sentido. Si pones los títulos con `<td>`, pierdes la negrita automática y la ayuda de accesibilidad. Cada etiqueta tiene su trabajo.
