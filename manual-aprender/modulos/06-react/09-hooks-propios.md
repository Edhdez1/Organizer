...
> Ya sabes usar los hooks que React trae de fábrica: `useState`, `useEffect`, `useContext`.
> En este capítulo das el último gran salto: **fabricar tus propios hooks**. Vas a aprender a
> tomar la lógica que se repite por toda tu app —pedir datos, saber quién está conectado,
> guardar un hábito— y meterla en una función reutilizable que empieza por `use`. Es
> exactamente así como están construidos RachaSimple y Faro por dentro. Bit, nuestro ajolote,
> ya tiene su frasco preparado para guardar lógica.

---

## 1. El problema: lógica que se repite

Imagina que en RachaSimple, cinco componentes distintos necesitan la lista de hábitos del
usuario. Cada uno tendría que: pedir los datos a Supabase, guardar un estado de "cargando",
manejar el error si algo falla, y volver a pedir cuando los datos cambian. Eso es un montón
de código, y copiarlo cinco veces es una pesadilla: si arreglas un fallo en uno, tienes que
acordarte de arreglarlo en los otros cuatro.

La solución de React tiene un nombre muy concreto: el **hook personalizado**.
...
> que por dentro usa otros hooks (como `useState`, `useEffect` o `useQuery`). Sirve para
> **empaquetar lógica reutilizable** y compartirla entre varios componentes sin copiar y
> pegar. En RachaSimple, el archivo `src/hooks/useHabits.ts` es un hook personalizado: cualquier
> componente que quiera la lista de hábitos solo llama `useHabits()` y ya.

La idea clave: un componente sirve para **dibujar** interfaz; un hook personalizado sirve para
**compartir lógica** (datos, cálculos, conexiones). No dibuja nada. Devuelve información y
funciones para que el componente las use.

> ### 💡 Tip
> Si te encuentras copiando el mismo bloque de `useState` + `useEffect` en dos componentes,
> esa es la señal de "aquí cabe un hook personalizado". No lo hagas a la primera copia; cuando
> aparezca la segunda o tercera, extrae.

---

## 2. La convención `use*`: por qué el nombre importa

Tu hook personalizado **tiene que** empezar por `use`. No es un capricho de estilo: React
revisa los nombres para aplicar sus reglas.

> ### 🟦 ¿Qué significa? — *Convención `use*`*
> Es la **regla de nombrado** de los hooks: cualquier función que use hooks por dentro debe
> llamarse `useAlgo` (`useHabits`, `useAuth`, `useCheckins`). React y las herramientas de
> análisis (el linter) usan ese prefijo para saber que dentro hay hooks y comprobar que los
> llamas bien. Si nombraras la función `getHabits` en lugar de `useHabits`, React no la
> trataría como hook y podrías romper las reglas sin que nadie te avise.

> ### ⚠️ Cuidado — Las reglas de los hooks siguen vigentes
> Un hook personalizado es un hook de verdad, así que hereda las dos reglas que ya viste:
...
> Por eso `useHabits()` no se puede llamar dentro de un `onClick`, por ejemplo. Si necesitas
> datos en un evento, llamas el hook arriba y usas su resultado dentro del evento.

> ### 🔎 En tu código
> Mira los nombres reales de RachaSimple: `useHabits`, `useActiveHabits`, `useHabit`,
> `useCheckins`, `useUpsertCheckin`, `useAuth`, `useI18n`. Todos empiezan por `use`. Ningún
> archivo de hooks rompe esa convención: es la firma visual de "esto comparte lógica".
...
hook que guarda un contador (la idea es la misma para cosas más serias).
...
llamas `useContador(10)` y listo, sin copiar nada.

> ### 🟦 ¿Qué significa? — *Extraer lógica*
> **Extraer** es sacar un trozo de código de un sitio donde estaba mezclado y ponerlo en su
...
> componente queda limpio: solo se ocupa de dibujar.

> ### 💡 Tip
> Cada vez que **llamas** un hook personalizado, obtienes un estado **independiente**. Si en una
> pantalla pones `useContador(0)` dos veces, son dos contadores separados; no comparten el
...
Un hook personalizado puede devolver lo que tú quieras. Lo más común son dos estilos:

- **Un objeto** (`{ valor, subir, bajar }`): cuando devuelves varias cosas con nombre. Es lo
  más legible y lo que más se usa en RachaSimple y Faro.
- **Un array** (`[valor, setValor]`): cuando son pocas cosas y quieres elegir el nombre al
...
> En un hook, ese retorno es el "menú" que ofreces al componente: los datos que puede leer y
> las funciones que puede llamar. Tú decides qué exponer y qué dejar escondido dentro del hook.

> ### 💡 Tip — Objeto vs array
> Devuelve un **objeto** cuando hay tres o más cosas, o cuando algunas son opcionales: así el
> componente toma solo lo que necesita (`const { valor } = useContador(0)`) y no le importa el
> orden. Reserva el **array** para pares como `[estado, setEstado]`, donde el orden es obvio.

Un detalle importante: las funciones que devuelves (como `subir`) son las que el componente
pondrá en un `onClick`. Por eso el hook es tan cómodo: te da el **dato** para mostrar y la
**acción** para cambiarlo, todo junto.
...
Ocho líneas. Y, sin embargo, hace dos cosas valiosísimas:
...
2. **Protege** con un error claro. Si alguien usa `useAuth()` fuera del `AuthProvider`, en vez
   de un fallo confuso recibe el mensaje "useAuth must be used within an AuthProvider".
...
> de error: solo recibe el usuario. Si mañana cambias cómo guardas la sesión, tocas un solo
> archivo y nadie más se entera. Eso es encapsular.

> ### 🔎 En tu código
> En RachaSimple, `useHabits.ts` y `useCheckins.ts` llaman `useAuth()` por dentro para saber
> de quién son los hábitos: `const { user } = useAuth();`. Así, un hook usa a otro hook. Eso te
> lleva directo a la siguiente idea: componer hooks.

---

## 6. Componer hooks: un hook que usa otros hooks

Aquí está lo más potente. Como un hook personalizado puede usar hooks por dentro, también
puede usar **otros hooks personalizados tuyos**. A eso se le llama **componer**.

> ### 🟦 ¿Qué significa? — *Componer hooks*
> **Componer** es construir un hook usando otros hooks como piezas. Igual que armas un mueble
> con tornillos y tablas ya hechos, armas `useHabits` usando `useQuery` (de TanStack Query) y
> `useAuth` (tuyo) por dentro. Cada pieza hace una cosa; al juntarlas obtienes algo más grande
> sin reinventar lo de abajo. En RachaSimple, `useCreateHabit` compone `useAuth`,
> `useMutation` y `useQueryClient`.
...
nada de esto: solo llama `useCreateHabit()` y obtiene una función para guardar.
...
> y `useMutation` (para **escribir**: crear, editar, borrar). Se encarga sola del estado de
> "cargando", de los errores y de recordar los datos. RachaSimple no la usa directamente en sus
...
> la clave `['habits']`: así la lista se refresca sola en toda la app sin que tú lo pidas.

---

## 7. Caso real: la familia `useHabits` de RachaSimple

El archivo `src/hooks/useHabits.ts` no tiene un solo hook: tiene **una familia** de hooks
pequeños, cada uno con un trabajo claro. Estos son los de **lectura** (versión real):
...
Y en `useCheckins.ts` se repite el mismo patrón para los registros diarios: `useCheckins`,
`useCheckinsByDate`, `useCheckinsForHabit`, `useUpsertCheckin`. La lección de diseño es:
**muchos hooks pequeños y enfocados** son mejores que uno gigante que hace de todo.

> ### 🟦 ¿Qué significa? — *`queryFn` y la capa de repositorios (`habitsRepo`)*
> La **`queryFn`** es la función que de verdad va a buscar los datos. Fíjate en que no contiene
> SQL ni llamadas a Supabase a la vista: llama a `habitsRepo.listAll()`. Ese `habitsRepo` es la
...
> si todavía no hay id". Esa precisión es lo que hace reutilizable un hook: hace una cosa, la
> hace bien, y el componente elige cuál de la familia necesita.
...
> los componentes devuelven interfaz. Mantén la línea clara.
...
complejidad vive en los hooks. El componente se lee casi como una frase: "trae los hábitos,
si cargan muéstrame 'Cargando', si no píntalos, y este botón crea uno". Esa claridad es el
premio de los hooks personalizados.

> ### 🟦 ¿Qué significa? — *`data`, `isLoading` y `.mutate()`*
> Cuando llamas un hook de TanStack Query obtienes un objeto con campos listos: **`data`** son
> los datos ya cargados (aquí, los hábitos), **`isLoading`** es `true` mientras llegan. En las
> mutaciones, **`.mutate(...)`** es la función que dispara la acción (crear el hábito). Tú no
> programaste esos nombres: vienen de la librería, y tus hooks te los pasan tal cual.

> ### 🔎 En tu código
> Faro (Next.js 15 + React 19) sigue la misma filosofía: encierra su lógica de análisis de
> proyectos en módulos propios para que los componentes `.tsx` queden enfocados en mostrar la
> descripción, el progreso y el roadmap, no en hablar con OpenAI o Supabase. Mismo principio
> que RachaSimple: el componente dibuja, el hook/servicio piensa.

---

## 9. ¿React o algo más sencillo? Saber cuándo NO usar un hook

No todo merece un hook, y no toda app es React. PolyPaw está hecho en Python con Flet, y
tunal-digital es HTML/CSS/JavaScript puro: ahí no existen los hooks. Y dentro de React, una
lógica que **no usa otros hooks** (un cálculo simple, por ejemplo "sumar dos números") no
necesita ser un hook: con una función normal basta.

> ### ⚠️ Cuidado — Un hook solo si usa hooks
> La regla práctica: crea un hook personalizado **únicamente** si por dentro vas a usar
> `useState`, `useEffect`, `useContext`, `useQuery` u otro hook. Si tu función solo recibe
> datos y devuelve datos sin tocar estado de React, hazla una **función normal** (sin el
> prefijo `use`) y guárdala en un archivo de utilidades. Reservar `use` para lo que de verdad
> es un hook evita confundir a React y a tu yo del futuro.
